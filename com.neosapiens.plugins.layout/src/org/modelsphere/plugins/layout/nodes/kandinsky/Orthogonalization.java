...
public class Orthogonalization {
     ...
     * Upwardplanarization G =(V,E,F), Mapping type : V →{vertex , crossing, hypervertex} Output:
    ...
    private void tailShape(UpwardPlaneStringGraph upwardPlaneStringGraph) {
        ...
        for (Vertex vertex : upwardPlaneStringGraph.vertexSet()) {
            /*
             * l = out(v), m = median(l), Ss(lm)=�? ↑ �? if type(v)= crossing then if m =0 then
             * Ss(l1)=�? →↑ �? else Ss(l0)=�? �?↑ �? else for 0 ≤ i<m { Ss(li)=�? ↑�?↑ �? } for m<i< |l| {
             * Ss(li)=�? ↑→↑ �? }
             */
        }
    }
    ...
    private void headShape(UpwardPlaneStringGraph upwardPlaneStringGraph) {
        ...
        for (Vertex vertex : upwardPlaneStringGraph.vertexSet()) {
            /*
             * l = in(v), m = median(l) if type(v)= crossing then if m =0 then Ss(l1)+=�? �? �? else
             * Ss(l0)+=�? → �? else if type(v)= hypervertex then for 0 ≤ i<m do Ss(li)+=�? → �? for m<i<
             * |l| do Ss(li)+=�? �? �? else for 0 ≤ i<m do Ss(li)+=�? →↑ �? for m<i< |l| do Ss(li)+=�? �?↑
             * �?
             */
        }
    ...
    private void assignShapeToReversedEdges(UpwardPlaneStringGraph upwardPlaneStringGraph) {
        ...
        for (Edge edge : edges) {
            /*
             * //E=e voir 104 if Ss(e)= E Ss(e)= Ss(¯e)
             */
        }
    ...
    private void assignAngles(UpwardPlaneStringGraph upwardPlaneStringGraph) {
        ...
        for (Vertex vertex : upwardPlaneStringGraph.vertexSet()) {
            /*
             * for 0 ≤ i< |out(v)|− 1 { Sa(out(v)i)=0 } if δ−(v)=0 Sa(out(v)|out(v)|−1)=4) else
             * Sa(out(v)|out(v)−1|)=2 for 1 ≤ i< |in(v)|− 1 { Sa(in(v)i)=0 } if δ+(v)=0
     ...
     * bends. A bend stretching transformation removes su- perﬂuous bends from an edge by assigning
     * a new shape with less bends to it without changing the �?rst and last direction in the shape.
